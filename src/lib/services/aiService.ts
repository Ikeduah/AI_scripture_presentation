import type { AppSettings, Template, LayoutType, Slide } from "@/lib/types";
import { ChatOpenAI } from "@langchain/openai";
import { ChatGroq } from "@langchain/groq";
import { HumanMessage, SystemMessage } from "@langchain/core/messages";
// @ts-expect-error - Known module resolution mismatch with next
...
export const generateSlidesFromText = async (
  ...
  try {
    if (provider === "openrouter") {
      llm = new ChatOpenAI({
        apiKey: apiKey,
        modelName: modelName || "anthropic/claude-3.5-haiku",
        temperature: 0.7,
        configuration: {
...
export const generateLogicSnippet = async (
  ...
  providerFromUser: "openrouter" | "groq" | undefined,
  ...
  let apiKey: string | undefined;
  if (provider === "openrouter") apiKey = appSettings.openRouterConfig?.apiKey;
  ...
  try {
    ...
    if (provider === "openrouter") {
      const llm = new ChatOpenAI({
        apiKey,
        modelName: modelFromUser || "anthropic/claude-3.5-haiku",
        temperature: 0.3,
        configuration: {
          baseURL: "https://openrouter.ai/api/v1",
          defaultHeaders: {
