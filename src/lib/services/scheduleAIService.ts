...
import type { ScheduleItem, AIScheduleResponse } from "@/lib/types/propresenter";
...
function getAIConfig(
  ...
  if (!apiKey) {
    ...
    if (provider === "openrouter" && appSettings.groqConfig?.apiKey) {
      return {
        provider: "groq",
        apiKey: appSettings.groqConfig.apiKey,
        model: "llama-3.3-70b-versatile",
    ...
    if (provider === "groq" && appSettings.openRouterConfig?.apiKey) {
      return {
        provider: "openrouter",
        apiKey: appSettings.openRouterConfig.apiKey,
        model: "anthropic/claude-3.5-haiku",
  ...
  try {
    ...
    if (config.provider === "openrouter") {
      const llm = new ChatOpenAI({
        ...
        modelName: "anthropic/claude-3.5-haiku", // Vision/parsing model via OpenRouter
        temperature: 0.3,
        configuration: {
