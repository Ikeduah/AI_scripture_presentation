...
import type { TranscriptionEngine, TranscriptionCallbacks, TranscriptionSegment, SmartVersesSettings } from "@/lib/types/smartVerses";
import { SMART_VERSES_SETTINGS_KEY, DEFAULT_SMART_VERSES_SETTINGS } from "@/lib/types/smartVerses";
import type RecordRTC from "recordrtc";
import {
  isNativeWhisperModelDownloaded,
  resolveNativeWhisperModelPath,
} from "./nativeWhisperModelService";
import { extractNewTranscriptionText } from "@/lib/utils/transcriptionOverlap";
import { processGroqAudioWrapper } from "../api/transcription";
...
export class AssemblyAITranscriptionService implements ITranscriptionService {
  readonly engine: TranscriptionEngine = "assemblyai" as any;
  ...
  private async getTemporaryToken(): Promise<string> {
    try {
      ...
      const token = await getAssemblyAITemporaryToken();
...
export class GroqTranscriptionService implements ITranscriptionService {
  ...
  private async startWebRTCCapture() {
    ...
    const RtcModule = await import("recordrtc");
    const RecordRTCBuilder = RtcModule.default || RtcModule;
    const { StereoAudioRecorder } = RtcModule;
    
    this.recorder = new RecordRTCBuilder(this.mediaStream, {
      type: "audio",
      mimeType: "audio/wav",
  ...
  private async sendToGroq(blob: Blob, sequence: number, captureTimestamp: number) {
    try {
      let data;
      try {
        data = await processGroqAudioWrapper(blob);
      } catch (err: any) {
...
export function createTranscriptionService(
  ...
  // @ts-ignore - Support legacy string values if they exist in storage
  switch (settings.transcriptionEngine as string) {
    case "groq":
    ...
    default:
      // Default to native whisper if unknown
      return new MacNativeWhisperTranscriptionService(
        settings.offlineWhisperNativeModel || "ggml-small.en-q5_1.bin",
        settings.offlineLanguage || "en",
        callbacks
