...
import type { GlobalAIResponse, AIAssistantContext, ExecutedAction, CreateSlidesParams, CreatePlaylistParams, SetTimerParams, TriggerSlideParams, SlideNavigationParams, ChangeStageLayoutParams, DisplayMessageParams, DisplayWithAITemplateParams, SetProPresenterTimerParams, ClearLayerParams, UpdateCurrentSlidesParams } from "@/lib/types/globalChat";
import type { Template, Playlist, Slide, AIProvider } from "@/lib/types";
import type { ScheduleItem } from "@/lib/types/propresenter";
import { getAppSettings } from "@/lib/utils/aiConfig";
import { loadProPresenterAITemplates } from "@/lib/utils/proPresenterAITemplates";
import { loadProPresenterConnections, getEnabledConnections } from "./propresenterService";
import { loadSmartAutomations } from "@/lib/utils/testimoniesStorage";
...
function getAIConfig(): AIConfig | null {
  ...
  if (appSettings.globalAssistantModel?.provider && appSettings.globalAssistantModel?.model) {
    ...
    if (provider === "openrouter") {
      apiKey = appSettings.openRouterConfig?.apiKey;
  ...
  if (appSettings.defaultAIProvider) {
    ...
    if (provider === "openrouter" && appSettings.openRouterConfig?.apiKey) {
      apiKey = appSettings.openRouterConfig.apiKey;
      model = appSettings.defaultAIModel || "anthropic/claude-3.5-haiku";
  ...
  if (appSettings.openRouterConfig?.apiKey) {
    return {
      provider: "openrouter",
      apiKey: appSettings.openRouterConfig.apiKey,
